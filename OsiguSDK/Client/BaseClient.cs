...
using OsiguSDK.Converters;
using OsiguSDK.Exceptions;
using OsiguSDK.Models;
using OsiguSDK.Models.Requests;
using RestSharp;

namespace OsiguSDK.Client
{
    public class BaseClient
    {
        ...
        protected BaseClient(Configuration configuration)
        {
        ...
        {
            var errorText = "Unexpected error occured.";
            try
            {
                var requestError = DeserializeStream<RequestError>(response.Content, "requestError");
            ...
            return new RequestException(errorText, (int)response.StatusCode);
        }
