...
namespace OsiguSDK.SpecificationTests.Products.Provider
{
    ...
    public class SubmitARemovalSteps
    {
        ...
        public void GivenIHaveTheProviderProductsClientWithoutTheRequiredPermission()
        {
            ...
            TestClients.ProductsProductsClientWithNoPermission = new ProductsClient(ConfigurationClients.ConfigProviderBranch2);
        }
        ...
        public void WhenIRequestTheSubmitARemovalEndpoint()
        {
            try
            {
                TestClients.ProductsProviderClient.SubmitRemoval(_productId ?? "not-existing-product");
            }
            catch (RequestException exception)
            {
                Responses.ErrorId = exception.ResponseCode;
        ...
        public void WhenIRequestTheSubmitARemovalEndpointWithoutPermission()
        {
            try
            {
                TestClients.ProductsProductsClientWithNoPermission.SubmitRemoval(Requests.SubmitProductRequest.ProductId);
            }
            catch (RequestException exception)
            {
                Responses.ErrorId = exception.ResponseCode;
